...
const AuthForm = props => {
  ...
  const handleFormSubmit = evt => {
    evt.preventDefault();
    const formName = isSignup ? 'signup' : name;
    const username = evt.target.username.value;
    const password = evt.target.password.value;
      ...
      <form onSubmit={handleFormSubmit} name={isSignup ? 'signup' : name} className="auth-form">
        ...
        {/* <div className="auth-button-group">
          ...
          {!isSignup ? (
            ...
            <button type="button" className="auth-button" onClick={() => setIsSignup(false)}>Login</button>
          )}
        </div> */}
        <div className="auth-button-group">
  <button type="submit" className="auth-button auth-button-login">{isSignup ? 'Sign Up' : 'Login'}</button>
  {!isSignup && (
    <React.Fragment>
      <button type="button" className="auth-button auth-button-signup" onClick={() => setIsSignup(true)}>Sign Up</button>
      <button type="button" className="auth-button auth-button-guest" onClick={handleGuestSubmit}>Continue as Guest</button>
    </React.Fragment>
  )}
  {isSignup && (
    <button type="button" className="auth-button auth-button-signup" onClick={() => setIsSignup(false)}>Login</button>
  )}
</div>
        {error && error.response && <div className="auth-error">{error.response.data}</div>}
        {localError && <div className="auth-error">{localError}</div>}
