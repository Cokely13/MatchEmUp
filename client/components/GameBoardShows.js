...
const GameBoardShows = () => {
  ...
  const shuffleShowsAndCharacters = () => {
    // Use allShows from the redux store instead of the hardcoded qbs array
    const shuffledShows = [...allShows].sort(() => 0.5 - Math.random());

    // Select first 4 QBs
    const selectedShows = shuffledShows.slice(0, 4);

    // Extract and shuffle WRs from the selected QBs
    const selectedCharacters = selectedShows.flatMap(show =>
      show.characters.sort(() => 0.5 - Math.random()).slice(0, 4)
    ).sort(() => 0.5 - Math.random());

    setGameWords(selectedCharacters.map(character => ({
       name: character.name,
  ...
  const handleSubmit = () => {
    if (selectedWords.size === 4) {
      ...
      const isSameShow = allShows.some((show) => {
        const matchingCharacters = selectedWordArray.filter((characterName) =>
        show.characters.some((character) => character.name === characterName)
        );

        if (matchingCharacters.length === 3) {
          matchingShows.push(show.name);
        }

        const allCharacterMatch = matchingCharacters.length === 4;

        if (allCharacterMatch) {
          showImages.push(show.imagePath);
          showNames.push(show.name) // Capture the QB's image path when a match is found
        }

        return allCharacterMatch;
      });

      if (isSameShow) {
        // Correctly guessed all WRs from the same QB
        const newSubmittedWords = [...submittedWords, ...selectedWordArray.map((characterName, idx) => ({ name: characterName, showImagePath: showImages[idx], showName: showNames[idx] }))];
        setRow1(true)
        ...
        images.push(showImages)
        answers.push(showNames)
        setPicture(images)
        setAnswer(answers)
        ...
        const remainingWords = gameWords.filter((character) => !selectedWords.has(character.name));
  ...
  {showConfetti && <Confetti />}
      ...
      <Modal show={showHowToPlayModal} onHide={handleClose} className="custom-modal">
        ...
        <Modal.Body>
          Select 4 Characters from the same show.
        </Modal.Body>
        <Modal.Footer>
