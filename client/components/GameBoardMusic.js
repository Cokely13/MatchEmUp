...
//     {submittedWords.length === 16 || mistakes == 5 ? <div> <div><Link to="/home" style= {{color: 'white'}}>Home</Link> </div> <div className="control-panel">
//           <button style={{marginBottom: '20px'}} className="btn btn-primary" onClick={handlePlayAgain}>Play Again</button>

//         </div></div> : <div className="control-panel">
...
const GameBoardQb = () => {
  ...
  const handleLoss = () => {
    ...
    // Set correct answers for display
    setCorrectAnswers(gameWords);
    console.log("HEY", gameWords)
    console.log("cre", correctAnswers)
    setShowLossModal(true);
  };
  ...
  return (
    <div className="music-container">
      ...
      <LossModal show={showLossModal} onHide={() => setShowLossModal(false)}>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowLossModal(false)}>Close</Button>
        </Modal.Footer>
      </LossModal>
      <ErrorModal show={showErrorModal} onHide={() => setShowErrorModal(false)} />
      <Error2Modal show={showError2Modal} onHide={() => setShowError2Modal(false)} />
